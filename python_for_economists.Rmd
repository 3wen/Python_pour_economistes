...
Comparativement à la version 2.7, la version 3.0 a apporté des modifications profondes. Il faut noter que Python 2.7 prendra "[sa retraite](https://pythonclock.org/)" le premier janvier 2020. Passée cette date, le support ne sera plus assuré.
...
| --------------: | --------------------------------------------: |
| `math.ceil(x)` | Plus petit entier supérieur ou égal à `x` |
| `math.copysign(x, y)` | Valeur absolue de `x` mais avec le signe de `y` |
| `math.floor(x)` | Plus grand entier inférieur ou égal à `x` |
...
Pour effectuer des tests logiques sur les éléments d'un tableau, `NumPy` propose des fonctions, répertoriées dans le Tableau\ \@ref(tab:numpy-operateurs-logiques). Le résultat retourné par l'application de ces fonctions est un tableau de booléens.
...
#### Création de séries à partir d'une liste
...
Pour ajouter une valeur, on utlise la méthode `concat()` de pandas. Ici, avec `s_num`, comme l'index est manuel, nous sommes obligés de fournir une série avec une valeur pour l'index également :
```{python, eval=T, echo=TRUE, error=TRUE}
s_num_2 = pd.Series([1], index = [2])
print("s_num_2 : \n", s_num_2)
s_num = pd.concat([s_num, s_num_2])
print("s_num : \n", s_num)
```

On note que la méthode `concat()` retourne une vue, et que pour répercuter l'ajout, il est nécessaire d'effectuer une nouvelle affectation.


En ayant une série avec un index numérique généré automatiquement, on peut préciser la valeur `True` pour le paramètre `ignore_index` de la méthode `concat()` pour indiquer de ne pas tenir compte de la valeur de l'index de l'objet que l'on ajoute :
```{python, eval=T, echo=TRUE, error=TRUE}
s = pd.Series([10, 2, 4])
s = pd.concat([s, pd.Series([2])], ignore_index=True)
print("s : \n", s)
```

#### Ajout de plusieurs valeurs dans une série

Pour ajouter plusieurs valeurs, on utlise la méthode `concat()`. Ici, avec `s_num`, comme l'index est manuel, nous sommes obligé de fournir une série avec une valeur pour l'index également :
```{python, eval=T, echo=TRUE, error=TRUE}
s_num_2 = pd.Series([1], index = [2])
s_num = pd.concat([s_num, s_num_2])
print("s_num : ", s_num)
...
Comme pour une série, pour ajouter une ligne, on utlise la méthode `concat()`. Dans un premier temps, créons un nouveau dataframe avec la ligne à ajouter :
```{python, eval=T, echo=TRUE, error=TRUE}
...
```{python, eval=T, echo=TRUE, error=TRUE}
df = pd.concat([df, nv_ligne], ignore_index=True)
```

À nouveau,la méthode `concat()` appliquée à un dataframe, retourne une vue et n'affecte pas l'objet.


On peut noter que lors de l'ajout d'une ligne, si le nom des colonnes n'est pas indiqué dans le même ordre que dans le dataframe dans lequel est effectué l'ajout, il faut rajouter une indication au paramètre `sort` de la méthode `concat()` :
...
print("avec sort=True : \n", 
  pd.concat([df, nv_ligne], ignore_index=True, sort=True))
```
...
Puis l'insertion :
```{python, eval=T, echo=TRUE, error=TRUE}
df = pd.concat([df,nv_lignes], ignore_index=True)
```
...
```{python, eval=T, echo=TRUE, error=TRUE}
print("min : ", df.height.min(numeric_only=True))
```
...
```{python, eval=FALSE, echo=TRUE, error=TRUE}
...
Pour coller les dataframes les uns en-dessous des autres, on peut utiliser la méthode `concat()` éaglement. 
...
```{python, eval=T, echo=TRUE, error=TRUE}
print(chomage.mean(numeric_only=True))
```
...
```{python, eval=T, echo=TRUE, error=TRUE}
chemin = "./fichiers_exemples/chomage.csv.gz"
...
Pour enregistrer les données d'un dataframe dans un fichier HDF5 utilisant HDFStore, `pandas` propose la méthode `to_hdf()` qui fonctionne de manière similaire à la fonction `to_csv()` (cf. Section\ \@ref(pandas-export_csv)). Make sure to install the module `tables` first (`pip install tables`).
...
chomage.to_hdf(chemin, key="base_chomage", index=False)
```
...
chomage = pd.read_excel(chemin, skiprows=2, header=1, sheet_name = 1)
print(chomage)
```
...
- `sheet` : nom ou numéro de feuille (attention à l'index qui commence à 0) ;
...
Les adeptes du logiciel et langage de programmation R seront heureux de pouvoir retrouver une grammaire graphique comme celle proposée par le [*package* R `ggplot2`](https://ggplot2.tidyverse.org/) introduit par Hadley Wickham. En effet, il existe une librairie Python nommée `ggplot` : http://ggplot.yhathq.com/.
...
Commme on peut le constater, la référence à la couleur peut se faire en faisant appel à son nom (la liste des couleurs disposant d'un nom est accessibles sur la [documentation de matplotlib](https://matplotlib.org/examples/color/named_colors.html)). On peut aussi utiliser un code hexadécimal pour faire référence à une couleur.

```{block2, type='remarque', echo=TRUE}
Il peut être intéressant, lorsque l'on choisit des couleurs, de penser à l'utilisation ensuite : est-ce pour l'écran ? Pour l'impression en niveaux de gris ? On peut également se demander si le choix effectué ne gênera pas la compréhension pour les daltoniens (qui sont environ 4% en France). Le site web [Color Brewer](http://colorbrewer2.org/) propose des choix de couleurs en fonctions de caractéristiques souhaitées comme celles mentionnées.
```
...
Pour changer de type de ligne, on modifie le troisième paramètre de la fonction. On l'a vu, par défaut, une ligne est tracée, ce qui revient à indiquer comme troisième paramètre : `'-'`. Le Tableau\ \@ref(tab:fmt-lignes) indique les différents format possibles pour la ligne.
...
Table: (#tab:fmt-lignes) Formats des lignes
...
On peut aussi spécifier le type de ligne à l'aide du paramètre `linestyle`, en lui indiquant une des valeurs renseignées dans le Tableau\ \@ref(tab:fmt-lignes-2) 
...
Table: (#tab:fmt-lignes-2) Formats des lignes via le paramètre `linestyle`
...
Table: (#tab:fmt-lignes-marqueurs) Formats des lignes
...
- `marker` : renseigne le type de marqueur (c.f. Tableau\ \@ref(tab:fmt-lignes-marqueurs)) ;
...
```{block2, type='remarque', echo=TRUE}
[La documentation en ligne](https://matplotlib.org/2.0.2/api/pyplot_api.html#matplotlib.pyplot.scatter) de la fonction `scatter()` mentionne que la fonction `plot()` (c.f. Section\ \@ref(matplotlib-plot)) est plus rapide pour effectuer des nuages de points dans lesquels la couleur ou la taille des points de varie.
```
...
Pour changer la forme des marqueurs, on l'indique via le paramètre `marker` (c.f. Tableau\ \@ref(tab:fmt-lignes-marqueurs) pour les valeurs possibles) :
...
##### Plusieurs séries sur un diagramme en bâtons
...
```{python, eval=F, echo=FALSE, error=TRUE}
plt.subplot(n_lignes, n_col, courant)
```
...
### Éléments de graphiques {#pyplot-elements-graphiques}
...
plt.plot(x, y_2)
plt.title("Représentation graphique de $y = x^2$ \net de $y = -x^2$")
plt.savefig("figs/pyplot/plot_titre.png")
plt.close()
```


```{r, echo=FALSE, fig.pos="H", fig.align="center"}
knitr::include_graphics("figs/pyplot/plot_titre.png")
```
...
plt.xlabel("Valeurs de $x$")
plt.ylabel("Valeurs de $y$")
plt.savefig("figs/pyplot/plot_axes_label.png")
plt.close()
```


```{r, echo=FALSE, fig.pos="H", fig.align="center"}
knitr::include_graphics("figs/pyplot/plot_axes_label.png")
```
...
plt.plot(x, y_2, label = "cube ($x^3$)");
plt.legend();
plt.savefig("figs/pyplot/plot_legende.png");
plt.close()
```


```{r, echo=FALSE, fig.pos="H", fig.align="center"}
knitr::include_graphics("figs/pyplot/plot_legende.png")
```


Pour préciser la position de la légende, on peut s'appuyer sur le paramètre `loc` dans la fonction `legend()`, en indiquant une valeur telle que reportée dans le Tableau\ \@ref(tab:pyplot-legendes-loc).

| Chaîne | Code | Description |
| -----------: | | -----------: | ------------------------------------------------:|
| `best` | `0` | Laisser Python optimiser le positonnement |
| `upper right` | `1` | En haut à droite |
| `upper left` | `2` | En haut à gauche |
| `lower left` | `3` | En bas à gauche |
| `lower right` | `4` | En bas à droite |
| `right` | ` 5` | À droite |
| `center left` | `6` | Centré au milieu à gauche |
| `center right` | `7` | Centré au milieu à droite |
| `lower center` | `8` | Centré en bas |
| `upper center` | `9` | Centré en haut |
| `center` | `10` | Centré |
...
plt.plot(x, y_2, label = "cube ($x^3$)")
plt.legend(loc = "lower center")
plt.savefig("figs/pyplot/plot_legende_pos.png")
plt.close()
```


```{r, echo=FALSE, fig.pos="H", fig.align="center"}
knitr::include_graphics("figs/pyplot/plot_legende_pos.png")
```
